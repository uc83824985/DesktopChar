...
export class PerformanceRuntimeEffectHandler {
  private readonly pending = new Map<number, Set<AbortController>>();
  private readonly port: PerformanceInferencePort;

  constructor(port: PerformanceInferencePort) {
    this.port = port;
  }

  handle(effect: RuntimeEffect, dispatch: (event: AvatarEvent) => void): boolean {
    if (effect.type === 'performance.infer') {
    ...
    if (effect.type === 'performance.cancel') {
