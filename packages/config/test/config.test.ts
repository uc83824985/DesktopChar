...
test('loads offline mock defaults and MCP binding variables', () => {
  ...
  assert.deepEqual(config.mcp, { toolName: 'voice.generate', timeoutMs: 1234, textArgument: 'input', format: 'mp3', voice: 'alice' });
});
...
test('rejects invalid TTS environment values', () => {
  ...
  assert.throws(() => loadTtsConfig({ DESKTOP_CHAR_TTS_FORMAT: 'aac' }), /wav, mp3/);
});
