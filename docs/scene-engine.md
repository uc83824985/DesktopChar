...
当前实现纯数据 Runtime、关系求解、世界/UI 渲染计划和测试，不包含任何具体场景、模型资源或应用 presenter。桌面 UI 的详细边界见 [桌面 UI 引擎层设计](desktop-ui-engine.md)。
...
- `uiSurfaces`：应用 presenter 的框架无关引用、显示层和输入策略；
...
UI Surface 与 RenderPart 共享 Actor 生命周期，但不把 React/Vue/DOM 固化进 Scene Engine。`buildSceneRenderFrame()` 会在同一 generation/revision 中输出 `drawItems` 和 `uiSurfaces`；应用 presenter 根据 Actor state、Behavior mode、transform 与 config 生成具体 UI。UI 事件通过纯数据映射返回 `actor.interacted`，因此 UI 仍只是事件源。显示层、输入策略、参考项目取舍和窗口测量边界见 [桌面 UI 引擎层设计](desktop-ui-engine.md)。
...
3. `Coverage/Picking`：与颜色和深度分离，只有显式开启交互的可见部件才分配 picking id。桌面 renderer 已实现鼠标附近小块像素的异步 PBO/fence 流水线回读；当前读取 Live2D 最终帧 alpha，Scene Renderer 接入后可复用同一机制读取最终 Picking pass，无需每帧回读完整缓冲。
...
- UI Surface 的确定性分层、状态投影、隐藏生命周期、事件回传和非法声明拒绝。
