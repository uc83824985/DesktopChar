...
3. `Coverage/Picking`：与颜色和深度分离，只有显式开启交互的可见部件才分配 picking id。桌面 renderer 已实现鼠标单像素的异步 PBO/fence 回读；当前读取 Live2D 最终帧 alpha，Scene Renderer 接入后可复用同一机制读取最终 Picking pass，无需每帧回读完整缓冲。
