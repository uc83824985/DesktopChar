...
参考仓库与 `DesktopChar` 平级，不进入主仓库构建或提交。初次浅克隆跳过 LFS 实体；
2026-07-23 已由 `performance-model-service/bootstrap.ps1` 下载并校验完整
safetensors 和 tokenizer，仍不进入 DesktopChar Git。
...
`PerformanceInferencePortV2` Transport Profile：
...
## 2026-07-24 动态表情目录可行性复核

结合当前模型制品和 Transformers Provider，动态角色表情目录已在不绑定具体模型、
不暴露 Live2D 文件名的前提下完成 v2 领域链路：

- Qwen3.5-2B 默认 non-thinking，适合短文本到短结构化结果的分类/重排任务；
- 官方支持 OpenAI-compatible 服务边界，vLLM 还支持 `--language-model-only`，因此
  `PerformanceInferencePortV2` 可以保持模型与推理框架无关；
- `chat_template.jinja` 支持 tools，只说明模板能够表达工具调用，不等于当前服务
  能做 JSON Schema 约束生成；
- 当前安装的 Transformers serving 明确拒绝 `response_format` 和 `tool_choice`；
  因此 Adapter 声明 `structuredOutput: "prompt-only"` 是准确的，不能把动态 schema
  稳定性建立在服务端强约束上；
- v2 已拆分 `ExpressionCatalogPlanningAdapter` 和 `PerformanceModelTransport`，Mao
  八项目录不会把 `exp_0x` binding 发送给模型；v1 的真实 CPU 结果仍只作为 Provider
  可运行基线；
- 复核时 `/health` 正常，但 RTX 3080 仅余约 523 MiB 显存。为避免干扰现有负载，
  本轮未追加 GPU 生成；质量与延迟仍须在独占或受控显存条件下验收。

推荐按四组实验逐级验证，而不是一次替换 Runtime：

1. 保留 v1 固定 emotion 作为基线；
2. Qwen 直接从完整角色表情目录返回 top-3 `expressionKey`；
3. Qwen 只输出 affect 向量，由确定性 Resolver 选资源；
4. 采用混合方案：Qwen 输出 affect + top-3，Resolver 综合 persona、scene、转场、
   cooldown 和带种子的权重完成最终决策。

Mao 当前只有 8 个表情，首轮应把完整目录放入 prompt，无需检索。目录扩大后再增加
规则/向量召回的 shortlist，并由同一 Qwen Adapter 重排。若 prompt-only JSON 的合法率
或延迟不达标，只替换 `PerformanceInferencePortV2` 背后的 Transport/Provider（优先验证带约束解码的
vLLM/SGLang），不改变 Runtime、Resolver 或角色 Profile。
