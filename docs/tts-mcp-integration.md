...
被选中的 TTS Profile 内，`lifecycle.type` 只允许 `external` 或 `managed`。
...
应用配置位于 `desktop-char.config.json` 的 `ttsMcp` 分区，只负责选择 Profile 名。具体 TTS MCP 配置位于独立的 `tts-mcp-profiles/<name>.json` 文件。
...
1. `${dirname(desktop-char.config.json)}/tts-mcp-profiles/<name>.json`
2. `${cwd}/tts-mcp-profiles/<name>.json`
