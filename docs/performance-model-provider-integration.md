...
`PerformanceInferencePortV2` 为 DesktopChar 返回表情和已有 Live2D 动作建议，不在
AvatarRuntime 或 Renderer 中增加模型名称特判。

当前动态角色领域契约为：

```text
Contract: desktop-char.performance-planning.v2
Domain adapter: expression-catalog-planning
...
AvatarRuntime
  ...
  -> Runtime identity / catalog / policy validation
  -> ExpressionResolver
  -> PerformanceTimeline
  -> logical expressionKey / allowed action
```
...
| 模块 | 负责 | 不负责 |
| --- | --- | --- |
| AvatarRuntime | 请求 identity、generation、segment/catalog revision、Resolver 状态、最终 cue 和呈现顺序 | Provider 协议、进程与模型实现 |
| ExpressionCatalogPlanningAdapter | 语义 prompt、动态 key 白名单、响应解析和归一化 | HTTP、启动脚本、Live2D binding |
| PerformanceModelTransport | HTTP 映射、deadline、取消和文本补全结果 | 角色语义和资源 |
| Performance Provider | 根据请求文本和能力目录返回语义建议 | 回复文本生成、动作执行、播放时钟和角色状态 |
| BehaviorPolicy | 置信度、白名单、动作数量和可执行时点 | 模型推理与进程管理 |
| CharacterProfile | expression descriptor 与 Live2D binding | 模型通信和请求调度 |
| Renderer | 执行 Runtime 下发的 expression/motion 命令 | 选择 expression/action |

Provider 返回 `closed-eye-smile`、`nod` 等当次动态白名单 ID，不返回 `exp_02`、motion group/index 或
Live2D 参数。语义 ID 到具体资产的映射始终留在角色 sidecar 和 Renderer Adapter。

## 2. 当前生命周期

`performanceInference.lifecycle` 已实现 `external` 和 `managed`。两者只改变 Provider
进程所有权，不改变 Adapter、请求或结果契约。
...
`managed` 表示 DesktopChar 拥有配置的入口进程。Electron main：

- 以 `shell: false` 启动 `lifecycle.start`；
- 轮询 `healthUrl`，只有 readiness 通过才把 Provider 标记为 operational；
- 周期检查 endpoint，入口异常退出或健康检查失败时按配置重启；
- 停用、配置替换和正常退出时关闭 owned 进程树；
- Windows 下由独立 host 监控 DesktopChar owner PID，主进程被强杀时也回收 Provider。

右键菜单会标记“表情动作推理（托管）”。勾选会启动 Provider 并进入 `starting`，健康
检查通过后进入 `ready`；取消勾选会停止 owned 进程。启动和结束逻辑只存在于
Supervisor，不进入领域 Adapter 或 OpenAI-compatible Transport。
...
| `lifecycle` | `external`/`managed` 字符串简写，或下述生命周期对象 |
| `provider` | 必需；写入日志和标准化建议的稳定实现标识 |
| `baseUrl` | 必需；当前应用配置只接受 loopback HTTP `/v1` 根地址 |
| `healthUrl` | readiness/周期健康检查 URL；默认 `{baseUrl}/models` |
...
| 字段 | 规则 |
| --- | --- |
| `type` | 必须为 `external` 或 `managed` |
| `start.executable` | 仅 managed 使用；入口程序的绝对路径或明确可解析路径 |
| `start.args` | 参数数组，不经 shell 拼接 |
| `start.cwd` | Provider 工作目录；生产配置建议绝对路径 |
| `start.env` | 只传给 Provider 入口的附加环境变量 |
| `startupTimeoutMs` | 等待 readiness 的上限；默认 180000 |
| `shutdownTimeoutMs` | 正常关闭进程树的上限；默认 10000 |
| `healthIntervalMs` | ready 后的周期检查间隔；默认 10000 |
| `restartOnFailure` | managed 异常退出/失去健康时是否重启；默认 true |

`external` 不使用 `start`。managed 省略 `start` 时，开发版默认启动仓库内
`performance-model-service/start.ps1`，端口从 `baseUrl` 推导。仓库 example 仍默认关闭
且使用 external，避免普通启动意外加载大模型。

配置文件支持热重载。新 revision 会取消旧 Adapter 的 in-flight 请求，后续 plan 使用
新 endpoint/参数；managed 的启动参数或 endpoint 变化会替换 owned 进程。旧 generation
或旧 segment revision 的迟到结果不能修改当前表演。
...
- external 不要求额外 readiness endpoint；managed 必须配置可用的 `healthUrl`，默认
  为 `{baseUrl}/models`。健康检查只能证明服务可接收请求，不能替代真实 Chat
  Completions 验收。
...
`requestId`、`planId`、`segmentId`、`segmentRevision`、`catalogRevision` 和
`generation` 不发送给模型。
...
- segment 已显式提供 expression 或兼容 emotion 时，模型不能替换；
...
3. 使用 DesktopChar 动态提供的 expression/action ID；
4. 先以独立进程启动 Provider；
5. 在 `desktop-char.config.json` 设置 endpoint 和可选 model；
6. 运行 Adapter 诊断；
7. 提交一个没有显式 expression/emotion/actions 的 segment；
8. 确认日志为 `request.v2-completed source:"model"`；
...
1. 若只是通信协议不同，实现新的 `PerformanceModelTransport`；
2. 若是专用分类器，直接实现 `PerformanceInferencePortV2`；
3. 保持 `PerformancePlanningRequestV2` 的语义；
4. 返回标准 `LocalPerformanceSuggestionV2`；
5. 保留 request/segment/catalog revision 和 AbortSignal；
...
- `packages/performance-inference/src/expression-catalog-adapter.ts`：v2 领域 Adapter；
- `packages/performance-inference/src/model-transport.ts`：通用模型 Transport 与
  OpenAI-compatible 实现；
- `packages/performance-inference/src/runtime-effect-handler-v2.ts`：v2 Effect/取消桥；
- `packages/performance-inference/src/rule-based-v2.ts`：不依赖模型的确定性实现；
- `packages/performance-inference/test/v2-adapter.test.ts`：解耦、白名单和回退测试；
- `scripts/performance-diagnostic.ts`：使用桌面同一 Adapter 的契约诊断。

Qwen3.5-2B 是参考 Provider，不是协议名称。替换模型不能改变
`desktop-char.performance-planning.v2` 的领域语义。
...
前台验收时，启动 `npm run desktop`，提交一个没有显式
expression/emotion/actions 的 plan，并检查：

```text
[performance] ... "event":"request.v2-started"
[performance] ... "event":"request.v2-completed" ... "source":"model"
...
16. external 停用和退出不会结束外部 Provider；
17. managed 正常退出、SIGINT 和主进程强杀后不残留入口 host 或 endpoint。
...
退出诊断要求用户配置已启用 managed，并要求 `healthUrl` 对应端口在测试前未被其他
Provider 占用。Windows 上强杀和 SIGINT 的 Electron 退出码可能不是 0；验收依据是
owned host 与 endpoint 均消失。完整模块边界和多模型迁移策略见
