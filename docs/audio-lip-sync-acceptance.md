...
格式为 24 kHz、单声道、`pcm_s16le`。每个提示音边界使用 12ms 淡入淡出避免爆音；流默认按 20ms PCM 块输出。播放器从同一批 PCM 样本的 20ms RMS 窗口计算原始电平，并乘以 `sqrt(2)` 还原正弦峰值尺度。Runtime 随后使用当前角色的 `LipSyncProfile.gain` 映射嘴型，默认 Mao 为 `2.5`，结果钳制到 `0..1`；这一处理不改变音频输出音量。
