...
7. 已实现首个内存多 Agent 框架：`AgentConnectionManager` 管理逻辑 Char worker 与并发额度，
   `ConversationRuntime` 支持多 Turn 并行、sealed 文本提前扇出 TTS/表现准备、顺序提交和
   单消费者播放。Electron 已提供左键“角色对话”前台和安全 IPC；Char 与 Router 共用一个
   managed Codex App Server，并以独立 ephemeral thread 隔离请求。配置已迁移到
   `agentProviders + agentRoles`，旧 `conversation` 和根级 `routing` 字段直接拒绝；Router
   ...
   已覆盖 Session 连续提交、Provider 严格失败、配置热切换、managed Codex Auto 路由及
