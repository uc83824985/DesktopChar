...
首版使用一个无边框透明 Electron `BrowserWindow`。平时窗口与角色显示区域同尺寸；打开
对话栏时，同一个窗口可临时在角色视口左侧或右侧扩展，不新建第二个窗口，也不迁移
Renderer、Live2D Runtime 或对话状态。角色移动等价于移动原生窗口；主进程始终保存完整
窗口 bounds 和稳定的角色 lane，两者可以相互换算，因此显示位置、点击坐标和原生窗口
包围盒不会逐渐漂移。
...
`interaction.conversationSidebar.preferredSide` 指定 `left` 或 `right`，默认右侧。打开角色
对话时，main 以角色 lane 到当前显示器工作区两边的距离作确定性选择：

1. 偏好侧能容纳完整 468 DIP 扩展时使用偏好侧；
2. 偏好侧不足而另一侧能完整容纳时自动翻转；
3. 两边都不足完整宽度时使用较宽一侧，并缩窄面板；
4. 较宽一侧仍不足 320 DIP 时不调整窗口，退回角色窗口内覆盖显示。
...
从进程外清除原生 `WS_EX_TOPMOST`，再确认 `WM_WINDOWPOSCHANGED` 事件路径在不改变
presentation request、opacity 或焦点的情况下恢复置顶。该测试没有常驻轮询兜底：
WindowProc 消息未被捕获会直接超时失败。它用于区分“窗口/渲染丢失”和“Electron 仍
报告 always-on-top，但原生 Z-order 已被截图工具修改”。
