...
- renderer 持续 watch 当前坐标：每个 fence 完成后自动提交下一帧查询，不再依赖 main 再发送一次鼠标移动。因此鼠标完全静止时，动作或模型形变造成的 Coverage 变化仍会更新穿透状态。
- 确认状态使用非对称防抖：连续 2 个可见样本进入选中，连续 3 个透明样本才退出。新查询处于 pending 时保持上一个确认状态，单帧透明边缘或角色内部移动不会闪回默认光标。
...
`npm run desktop` 会先构建再启动透明悬浮窗口。`body` 的 `data-pixel-readback` 会显示 `async-pbo` 或 `sync-one-pixel`；`data-pixel-sample` 是最新原始样本，`data-pixel-selection` 是防抖后的确认状态，并额外记录 alpha、命中/透明连续计数与 fence 延迟帧数。自动烟雾测试会验证窗口透明/置顶/尺寸、像素读取适配器已启用、初始穿透、拖动后原生 bounds，以及 renderer 收到的 bounds 是否一致。
