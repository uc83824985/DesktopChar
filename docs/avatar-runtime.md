...
播放器只拥有媒体对象和媒体播放状态。Runtime 收到 `playback.started` 后自行决定状态是否转为 `speaking`。
...
- pause、resume、stop；
- 音频资源和 object URL 清理；
- 输出 amplitude envelope 或提供可采样音频数据；
...
其中 `ParamMouthOpenY` 默认由 Mouth 独占；Gesture 可以临时接管头部参数；Gaze 在无强 Gesture 时影响头部；Gesture 结束后必须平滑释放参数所有权。
