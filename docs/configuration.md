...
当 Electron 托管该参考 Provider 时，相同参数从应用 JSON 的 `ttsMcp.local` 传入；只有用户在命令行独立运行 `local-tts-mcp` 时才需要上述变量。若后续为独立服务增加配置文件，应使用它自己的 `local-tts-mcp.config.json`，而不是让服务反向依赖 DesktopChar 的整份配置。
...
- `DESKTOP_CHAR_TTS_MODE`、`DESKTOP_CHAR_TTS_MCP_ENABLED` 及全部 `DESKTOP_CHAR_TTS_*` 连接/格式变量
