...
`activeMode=mcp` 是正确结果：`managed` 表示 DesktopChar 负责 Provider 进程生命周期，实际控制面仍只使用正式 MCP 协议。若要对照四音高版本，在应用 JSON 的 `ttsMcp.synthesis.voice` 选择 `jrpg-blip-varied` 后启动即可。
