...
本地 Provider 默认故意不返回 `duration_ms`、`amplitude` 和 `text_cues`，以覆盖当前真实 Qwen3-TTS MCP 的未知总时长流，以及冒泡按 `charactersPerSecond` 降级的路径。需要验证扩展元数据时可在 Provider 契约测试中显式打开，而不是改变应用运行分支。
