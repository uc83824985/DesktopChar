...
播放器不修改或伪造电平。Mao 资产 Profile 中的 `LipSyncProfile.gain` 为 `2.5`，Runtime 将当前提示音约 `0.224` 的原始电平映射到约 `0.56` 的 `ParamMouthOpenY`，最后钳制到 `0..1`。增益跟随模型资产，只改变嘴型，不改变 PCM 和实际播放音量。
