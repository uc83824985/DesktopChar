...
默认离线 Mock 返回确定性的 24 kHz PCM 流描述，不启动真实 HTTP 服务；普通“模拟说话”仍由前台测试壳模拟缓冲、播放时钟和实时电平。

独立的“口型同步验收”使用代码生成的真实 24 kHz PCM 铃声分片和 `WebAudioPcmStreamPlayer`：可听声音、`playback.level` 和模型参数验收都来自同一份 PCM，不使用 Mock amplitude。详细先验时间表和阈值见 [先验铃声流与口型时点验收](audio-lip-sync-acceptance.md)。关键配置见 `.env.example`：
