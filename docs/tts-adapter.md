...
默认 `managed` 生命周期会执行仓库根目录独立 [`local-tts-mcp`](../local-tts-mcp/README.md) 的服务入口。该服务使用官方 SDK 的 Streamable HTTP transport，通过真实 TCP 端口暴露 `initialize`、`tools/list`、`tools/call` 和 session 生命周期，并通过另一个 HTTP endpoint 分块输出 PCM。`external` 只改变服务器所有权，不改变 Client、Adapter 或数据面。Renderer 始终只看到 IPC 后的 `McpClientPort`、`McpTtsAdapter` 和 HTTP `AudioStreamSource`。固定工具语义见 [TTS MCP 接入指南](tts-mcp-integration.md)，启停和热重载见 [MCP 服务生命周期](mcp-services.md)。
...
独立的“口型同步验收”通过 Provider input schema 明确声明的 `test_fixture: "known-tone-v1"` 请求 PCM fixture，不再用特殊语音文本、URI 或专用播放器旁路。它仍通过真实 `tts_open_stream -> HTTP stream_url -> AudioStreamSource -> WebAudioPcmStreamPlayer`；前台仅在 `tts_status.capabilities.test_fixtures` 声明该值时注入，不按 Local、Qwen 或生命周期特判。详细先验时间表和阈值见 [先验铃声流与口型时点验收](audio-lip-sync-acceptance.md)。Local Provider 启动参数见 `.env.example`：
